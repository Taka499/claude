...
> **This is the canonical copy of the ExecPlan methodology.** It is deployed to `~/.claude/docs/PLANS.md`, so it is readable from any project on this machine — including projects that have not checked in a copy of their own. A project that *does* check one in (`docs/PLANS.md` or `_docs/PLANS.md`, per that repo's layout) takes precedence for that repo, and its copy should be synced from this file rather than edited independently. `Taka499/project-template` ships a copy for exactly that reason; when this file changes, that copy is the follow-up. Global `CLAUDE.md` § ExecPlans and ADRs is what makes reading this mandatory before writing a plan.
