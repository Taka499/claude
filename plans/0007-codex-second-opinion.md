...
- ~~Network probe from sandboxed Bash: `api.openai.com` and `chatgpt.com/backend-api/codex` both `ENOTFOUND`.~~ **Retracted 2026-08-29**: that probe used Node's `fetch`, which ignores the proxy environment the sandbox routes allowed traffic through, and returns `ENOTFOUND` for permitted hosts too. It was never evidence. See `plans/0008` § A probe that measured the wrong thing — and note the decision it was cited to support (run Codex unsandboxed) has since been reversed.
