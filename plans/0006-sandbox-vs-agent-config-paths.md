...
> Extended on 2026-09-24: **`.git/config` is protected too**, and that is a second trigger independent of the working tree — `branch -d`, `branch -m`, `remote` and `config` rewrite tracking sections while touching no tracked file at all. Measured by probe in this clone: writing `.git/probe-plain.txt`, `.git/foo.lock`, `.git/myconfig`, `.git/settings.json`, `./config` and `./config.lock` all **succeed** sandboxed; only `.git/config` and `.git/config.lock` are refused with `Operation not permitted`. So it is neither `.git/` nor the `.lock` extension nor the name `config` that is guarded — it is that exact pair of paths. A freshly `git init`-ed repo under `$TMPDIR` accepts `git config --add` sandboxed, so the protection tracks *this repository*, not git in general; the inferred reason (not probed) is the one this whole plan turns on — the repo is the live agent configuration, and `.git/config` is what decides where its code comes from.
>
> The operational sting is that this failure is **partial and reports success**. `git branch -d feat/scaffold-project-skill` run sandboxed on 2026-09-24 printed `error: could not lock config file .git/config` and `warning: update of config-file failed`, then `Deleted branch feat/scaffold-project-skill (was 0da09e7)` and exited 0. The ref was gone; the `[branch "feat/scaffold-project-skill"]` section survived and had to be removed with `git config --remove-section` outside the sandbox. Audit with `git config --get-regexp '^branch\.'` after any sandboxed branch surgery — unlike the truncation failure above, nothing here looks broken.
